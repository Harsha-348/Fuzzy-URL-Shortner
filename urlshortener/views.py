...
def redirect_to_original(request, short_code):
    ...
    now = timezone.now()
    print(f"[DEBUG] Now: {now}, Expires at: {expires_at}")  # For interview troubleshooting

    if expires_at and now > expires_at:
        return HttpResponse("""
            <html><head><title>Expired Link</title></head>
            <body style='font-family:sans-serif;text-align:center;margin-top:50px;'>
            <h2>🚫 This short URL has expired.</h2>
    ...
    if url_obj.password:
        if not entered_password:
            ...
            return HttpResponse(f"""
                ...
                <body style='font-family:sans-serif;text-align:center;margin-top:50px;'>
                <h2>🔒 This short URL is password protected.</h2>
                <form method='get'>
            ...
            """, status=401)
        elif entered_password != url_obj.password:
            # Show form with error
            return HttpResponse(f"""
                <html><head><title>Password Incorrect</title></head>
                <body style='font-family:sans-serif;text-align:center;margin-top:50px;'>
